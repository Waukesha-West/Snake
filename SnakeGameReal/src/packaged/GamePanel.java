...
public class GamePanel extends JPanel implements ActionListener {
	...
	private static final long serialVersionUID = 6514213769248997785L;
...
// try to fix apple spawn in snake
...
public void page(Graphics g) {
		
		
		g.setColor(Color.WHITE);
		g.setFont(new Font("Ink Free", Font.BOLD, 75));
		FontMetrics metrics = getFontMetrics(g.getFont());
		g.drawString("Press F To Continue", (SCREEN_WIDTH - metrics.stringWidth("Press F To Continue"))/2 , SCREEN_HEIGHT/2);
...
public void snakeSkin(Graphics g) {
	
	for(int i = 0; i< bodyParts;i++) {
		if(i == 0) {
	g.setColor(new Color(5,60,20)); 
	...
	if (running == false) {
		...
		//&& DPFix3
		//&& DPFix4
		if (applesEaten == 2 || applesEaten == 11 || applesEaten == 16 || applesEaten == 25  || applesEaten == 31 ) {
			DP1 = true;
		}
...
public void gameOver(Graphics g) {
		...
		if(z == 0) {
		System.out.println("Player Has Lost At " + appleCountPrint + " Apples");
		z++;
		}
	}
	...
	if (x[i] == bombX && y[i] == bombY || x[i] == bomb2X && y[i] == bomb2Y ) {
...
changeColor++;
	...
	case KeyEvent.VK_Q:
//		if (changeKeys == 1 || changeKeys == 3 || changeKeys == 5 || changeKeys == 7 || changeKeys == 9 || changeKeys == 11 || changeKeys == 13 || changeKeys == 15 || changeKeys == 17 || changeKeys == 19 ) {
//			
//			
//			switch(direction) {
//			case 'U':
...
//changeKeys++;
//		}
//		else if (changeKeys == 0 || changeKeys == 2 || changeKeys == 4 || changeKeys == 6 || changeKeys == 8 || changeKeys == 10 || changeKeys == 12 || changeKeys == 14 || changeKeys == 16 || changeKeys == 18  ) {
//		
//			switch(direction) {
//			case 'U':
//			y[0] = y[0] - UNIT_SIZE;
